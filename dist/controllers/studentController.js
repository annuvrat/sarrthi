...
export const createStudent = async (req, res) => {
    ...
    sendSuccess(res, 'Student created', {
...
export const listStudents = async (req, res) => {
    ...
    const match = {};
    if (status)
        match.performanceStatus = status;
    if (mentorId)
...
export const updateStudent = async (req, res) => {
    ...
    const student = await Student.findByIdAndUpdate(req.params.id, { ...(notes !== undefined && { notes }), ...(mentorId !== undefined && { mentorId }), ...(attendancePct !== undefined && { attendancePct }) }, { new: true }).populate('userId', 'name email');
    if (!student)
        throw new AppError('Student not found', 404);
