...
let cache = null;
export const updateStudentMetrics = async (studentUserId) => {
    ...
    await Student.findOneAndUpdate({ userId: studentUserId }, {
...
export const getAnalytics = async () => {
    ...
    const [totalStudents, totalMentors, totalEvaluators, activeStudents, pendingEvaluations, completedEvaluations, performanceDistribution, taskTrend, scoreTrend,] = await Promise.all([
    ...
    const data = {
        ...
        charts: {
            ...
            taskCompletionTrend: taskTrend.map((t) => ({
                week: t._id,
                assigned: t.assigned,
                completed: t.completed,
            })),
            evaluationScoreTrend: scoreTrend.map((s) => ({
                week: s._id,
