...
import { Evaluation, Notification, Student, StudyPlan, Submission, Task, User, } from './models/index.js';
const seed = async () => {
    await connectDB();
    await Promise.all([
        User.deleteMany({}),
    ...
    const studentUsers = await User.insertMany(Array.from({ length: 10 }, (_, i) => ({
        name: `Student ${i + 1}`,
