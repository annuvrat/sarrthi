...
export const createSubmission = async (req: Request, res: Response) => {
  ...
  if (existing) throw new AppError('Submission already exists for this task', 400);
  ...
  if (mentor) {
    await createNotification(
      mentor._id.toString(),
      'submission_received',
      `New submission received for task: ${task.title}`,
  ...
  await notifyRole(
    'evaluator',
    'submission_received',
    `New submission pending evaluation for task: ${task.title}`,
    submission._id.toString()
  );
...
export const markTaskCompleted = async (req: Request, res: Response) => {
  ...
  const task = await Task.findOneAndUpdate(
    { _id: taskId, studentId: req.user!._id },
    { status: 'completed' },
    { new: true }
  );
  if (!task) throw new AppError('Task not found', 404);
