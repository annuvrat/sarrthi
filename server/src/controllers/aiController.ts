...
export const createStudyPlan = async (req: Request, res: Response) => {
  const studentUserId =
    req.user!.role === 'student' ? req.user!._id.toString() : (req.body.studentId as string);
  if (!studentUserId) throw new AppError('Student ID required', 400);
...
export const listStudyPlans = async (req: Request, res: Response) => {
  const studentUserId =
    req.user!.role === 'student' ? req.user!._id : (req.query.studentId as string | undefined);
